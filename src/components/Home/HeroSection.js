...
const HeroSection = () => {
  ...
  return (
    <div className="relative">
      {/* Hero Section */}
      <section className="relative pt-32 pb-80 bg-black overflow-hidden md:px-20 md:pt-40 md:pb-64 lg:pb-80">
      <div className="absolute inset-0 overflow-hidden">
          <div
            className="absolute inset-0 bg-black opacity-50"
            aria-hidden="true"
        ...
        <div className="relative container mx-auto px-4 flex flex-col md:flex-row items-center justify-between z-10">
          {/* Left Section */}
          <motion.div
            className="w-full md:w-3/5 mb-8 md:mb-0 text-white text-center md:text-left"
            initial={{ opacity: 0, x: -50 }}
            animate={{ opacity: 1, x: 0 }}
            transition={{ duration: 0.8 }}
          >
            ...
            {/* Typing and erasing text animation */}
            <h1 className="text-3xl sm:text-3xl md:text-4xl lg:text-6xl font-bold mb-6">
              Transform Your Passion into <span className="text-red-500">{displayedText}</span>
              <span className="blinking-cursor animate-blink-fast">|</span>
            </h1>
          ...
          <div className="w-full md:w-2/5 flex justify-center">
            <Image src="/cover-image.png" alt="User" width={600} height={600} className="w-4/5 md:w-full" />
          </div>
        </div>
      </section>
